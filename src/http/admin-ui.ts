  ...
  <main class="page">
    ...
    <section class="main-grid">
      <article class="card connection-card">
        ...
        <div class="quick-note"><strong id="connection-message">Đang chuẩn bị kết nối tự động…</strong><span>Mở app là tunnel tự chạy. Khi URL xuất hiện, chỉ cần bấm Sao chép và dán vào ChatGPT.</span></div>
        <label class="field featured">
          <span>URL dán vào ChatGPT</span>
          <div class="input-action"><input id="connector-url" type="text" readonly placeholder="Đang chờ tunnel tạo URL…"><button id="copy-connector" class="button primary" disabled>Sao chép URL</button></div>
          <small id="connector-hint">URL đầy đủ sẽ tự hiện tại đây.</small>
        </label>
        <button id="test-connection" class="button ghost wide test-conn-btn">⚡ Kiểm tra Máy local ↔ ChatGPT</button>
        ...
        <details class="advanced-panel"><summary>Tùy chọn tunnel và cố định 1 Link MCP</summary>
        <p class="muted">Mặc định tunnel tạo URL tạm ngẫu nhiên. Để <b>cố định 1 link duy nhất mãi mãi</b>, nhập Token Cloudflare hoặc Static Domain Ngrok phía dưới.</p>
        <div class="button-row tunnel-controls">
          <select id="tunnel-provider" aria-label="Nhà cung cấp tunnel">
            <option value="cloudflared" selected>Cloudflare · Tự động / Named Token</option>
            <option value="ngrok">Ngrok · Static Domain miễn phí</option>
            <option value="pinggy">Pinggy · Dự phòng</option>
          </select>
          <button id="start-tunnel" class="button primary">▶ Chạy lại</button>
          <button id="stop-tunnel" class="button danger" disabled>■ Dừng tunnel</button>
        </div>
        <p id="tunnel-error" class="inline-error" hidden></p>
        
        <label class="field">
          <span>Cloudflare Tunnel Token (để cố định 1 link Cloudflare)</span>
          <input id="cf-tunnel-token" type="password" placeholder="eyJhIjoi... (để trống nếu dùng Quick Tunnel tự động)">
        </label>

        <label class="field">
          <span>Ngrok Static Domain (Domain tĩnh miễn phí từ ngrok.com)</span>
          <input id="ngrok-domain" placeholder="ví dụ: my-static-mcp.ngrok-free.app">
        </label>

        <label class="field">
          <span>Ngrok Authtoken</span>
          <input id="ngrok-token" type="password" placeholder="Token từ dashboard ngrok.com">
        </label>
        ...
        <div class="mini-steps">
          <span><b>1</b> Mở app</span><span><b>2</b> Sao chép URL</span><span><b>3</b> Dán vào ChatGPT</span>
        </div>
        </details>
      </article>
...
const state={status:null,settings:null,tunnel:null,secret:null,tokenVisible:false,connectorPublicUrl:null};
...
function renderSettings(){const s=state.settings;if(!s)return;q("#workspace-path").value=s.workspacePath;q("#allow-write").checked=s.permissionMode==="workspace-write";q("#allow-destructive").checked=s.allowDestructive;q("#allow-remote-git").checked=s.allowRemoteGit;q("#allow-shell").checked=s.allowUnsafeShell;q("#allow-sensitive").checked=s.allowSensitiveFiles;q("#pick-folder").disabled=!s.folderPickerSupported;if(s.cloudflareTunnelToken)q("#cf-tunnel-token").value=s.cloudflareTunnelToken;if(s.ngrokDomain)q("#ngrok-domain").value=s.ngrokDomain;if(s.ngrokAuthToken)q("#ngrok-token").value=s.ngrokAuthToken;if(s.tunnelProvider)q("#tunnel-provider").value=s.tunnelProvider;syncPermissionControls()}
...
function renderTunnel(){const t=state.tunnel||{state:"stopped"};const badge=q("#tunnel-badge");const select=q("#tunnel-provider");const labels={stopped:"Chưa chạy",starting:"Đang lấy URL…",running:"Đã kết nối",stopping:"Đang dừng…",failed:"Có lỗi"};if(t.provider)select.value=t.provider;else if(state.settings&&state.settings.tunnelProvider)select.value=state.settings.tunnelProvider;badge.textContent=labels[t.state]||t.state;badge.className="status-pill "+(t.state==="running"?"good":t.state==="failed"?"bad":t.state==="starting"||t.state==="stopping"?"warn":"");const busy=t.state==="starting"||t.state==="running"||t.state==="stopping";select.disabled=busy;q("#start-tunnel").disabled=busy;q("#stop-tunnel").disabled=t.state==="stopped";q("#pinggy-url").value=t.publicUrl||"";q("#copy-pinggy").disabled=!t.publicUrl;const error=q("#tunnel-error");error.hidden=!t.error;error.textContent=t.error||"";const messages={stopped:state.settings&&state.settings.autoStartTunnel?"Tunnel tự động chưa chạy — bấm Chạy lại":"Tunnel chưa chạy",starting:"Đang tự tạo URL, vui lòng chờ vài giây…",running:"✓ URL đã sẵn sàng — sao chép và dán vào ChatGPT",stopping:"Đang dừng tunnel…",failed:"Không tạo được URL — mở Tùy chọn tunnel để thử lại"};q("#connection-message").textContent=messages[t.state]||"Đang kiểm tra kết nối…";if(state.connectorPublicUrl&&state.connectorPublicUrl!==t.publicUrl){state.secret=null;state.connectorPublicUrl=null}renderSecrets()}
function renderSecrets(){const secret=state.secret;q("#mcp-token").type=state.tokenVisible?"text":"password";q("#mcp-token").value=secret&&state.tokenVisible?secret.mcpToken:"••••••••••••••••••••••••";q("#reveal-token").textContent=state.tokenVisible?"Ẩn":"Hiện";const connector=secret&&secret.connectorUrl?secret.connectorUrl:"";q("#connector-url").value=connector;q("#copy-connector").disabled=!connector;const hint=q("#connector-hint");hint.textContent=connector?"Giữ URL này bí mật vì nó chứa token truy cập.":state.tunnel&&state.tunnel.publicUrl&&state.settings&&!state.settings.allowUrlToken?"URL nhanh đang tắt. Dùng OpenAI Secure MCP Tunnel hoặc bật ALLOW_URL_TOKEN.":"URL đầy đủ sẽ tự hiện tại đây."}
...
async function syncConnectorUrl(){const publicUrl=state.tunnel&&state.tunnel.publicUrl;if(!publicUrl){state.secret=null;state.connectorPublicUrl=null;renderSecrets();return}if(state.connectorPublicUrl===publicUrl)return;try{state.secret=await ensureSecret(true);state.connectorPublicUrl=publicUrl;renderSecrets()}catch(error){toast("Không tải được URL đầy đủ: "+error.message,true)}}
async function load(){try{const values=await Promise.all([api("/api/status"),api("/api/settings"),api("/api/tunnel"),api("/api/tasks"),api("/api/audit"),api("/api/checkpoints")]);state.status=values[0];state.settings=values[1];state.tunnel=values[2];state.tasks=values[3].tasks;state.events=values[4].events;state.checkpoints=values[5].checkpoints;renderStatus();renderSettings();renderTunnel();renderRows();await syncConnectorUrl()}catch(error){toast(error.message,true)}}
q("#refresh").addEventListener("click",load);
q("#allow-write").addEventListener("change",syncPermissionControls);
q("#start-tunnel").addEventListener("click",async()=>{const provider=q("#tunnel-provider").value;const label=provider==="cloudflared"?"Cloudflare":provider==="ngrok"?"Ngrok":"Pinggy";try{state.secret=null;state.connectorPublicUrl=null;state.tunnel=await api("/api/tunnel/start",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({provider})});renderTunnel();toast("Đang mở "+label+", chờ URL xuất hiện…")}catch(error){toast(error.message,true)}});
q("#stop-tunnel").addEventListener("click",async()=>{try{state.tunnel=await api("/api/tunnel/stop",{method:"POST"});state.secret=null;state.connectorPublicUrl=null;renderTunnel();toast("Đã dừng tunnel")}catch(error){toast(error.message,true)}});
...
for(const id of ["#allow-write","#allow-destructive","#allow-remote-git","#allow-shell","#allow-sensitive","#cf-tunnel-token","#ngrok-domain","#ngrok-token","#tunnel-provider"])q(id).addEventListener("change",()=>q("#save-state").textContent="Chưa lưu");
q("#save-settings").addEventListener("click",async()=>{const button=q("#save-settings");button.disabled=true;try{const body={workspacePath:q("#workspace-path").value,permissionMode:q("#allow-write").checked?"workspace-write":"read-only",allowDestructive:q("#allow-destructive").checked,allowRemoteGit:q("#allow-remote-git").checked,allowUnsafeShell:q("#allow-shell").checked,allowSensitiveFiles:q("#allow-sensitive").checked,tunnelProvider:q("#tunnel-provider").value,cloudflareTunnelToken:q("#cf-tunnel-token").value,ngrokDomain:q("#ngrok-domain").value,ngrokAuthToken:q("#ngrok-token").value};const result=await api("/api/settings",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(body)});state.settings=result.settings;state.secret=null;q("#save-state").textContent="Đã lưu";await load();toast("Đã lưu và áp dụng cài đặt!")}catch(error){toast(error.message,true)}finally{button.disabled=false}});
...
q("#test-connection").addEventListener("click",async()=>{const button=q("#test-connection");const panel=q("#test-connection-panel");const summary=q("#test-panel-summary");const stepsBox=q("#test-panel-steps");button.disabled=true;button.textContent="⏳ Đang kiểm tra Máy local ↔ ChatGPT…";panel.hidden=false;summary.textContent="Đang kiểm tra kết nối…";summary.className="";stepsBox.replaceChildren(el("div","test-step-loading","Đang kiểm tra Local Server, Public Tunnel và MCP Handshake…"));try{const res=await api("/api/test-connection",{method:"POST"});summary.textContent=(res.ok?"✓ ":"! ")+res.summary;summary.className=res.ok?"good":"warn";const steps=[res.steps.localServer,res.steps.tunnel,res.steps.protocol,res.steps.chatgpt];stepsBox.replaceChildren(...steps.map(s=>{const row=el("div","test-step "+(s.ok?"ok":"fail"));const top=el("div","test-step-head");top.append(el("strong","",(s.ok?"✓ ":"✕ ")+s.name));if(s.latencyMs!==undefined)top.append(el("span","latency-pill",s.latencyMs+"ms"));row.append(top,el("small","",s.detail));return row}));toast(res.ok?"Kết nối Máy local ↔ ChatGPT hoạt động":"Kiểm tra xong: có mục cần chú ý",!res.ok)}catch(error){summary.textContent="✕ Lỗi kiểm tra kết nối";summary.className="bad";stepsBox.replaceChildren(el("div","test-step fail",error.message));toast(error.message,true)}finally{button.disabled=false;button.textContent="⚡ Kiểm tra Máy local ↔ ChatGPT"}});
q("#close-test-panel").addEventListener("click",()=>{q("#test-connection-panel").hidden=true});
setInterval(async()=>{if(!state.tunnel)return;try{state.tunnel=await api("/api/tunnel");renderTunnel();await syncConnectorUrl()}catch{}},2000);
load();
